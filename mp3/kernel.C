...
int main() {
    ...
    SimpleFramePool kernel_mem_pool(KERNEL_POOL_START_FRAME,
                                  KERNEL_POOL_SIZE,
                                  0);

    //unsigned long n_info_frames = SimpleFramePool::needed_info_frames(PROCESS_POOL_SIZE);
    
    unsigned long process_mem_pool_info_frame = kernel_mem_pool.get_frame();
    
    SimpleFramePool process_mem_pool(PROCESS_POOL_START_FRAME,
                                   PROCESS_POOL_SIZE,
