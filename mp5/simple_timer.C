...
/* What to do when timer interrupt occurs? In this case, we update "ticks",
   ...
   when the system gets initialized. (e.g. in "kernel.C") */
    ...
    // Please uncomment to test the FIFO feature lines 67-74
   /*
    if (ticks >= hz )
    {
        seconds++;
    ...
    */
    ...
    // RR Scheduler Timer Functionality 
    // Please comment this to check FIFO functionality lines 81-97
    
    if (ticks >= hz)
    {
        seconds++;
        ticks = 0;
		// give the CPU to another thread whenever a quantum has elapsed
